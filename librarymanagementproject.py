...
def process_payment(amount):
    ...
    while True:
        account_number = input("Enter your card number (10-15 digits): ")
        if is_valid_account_number(account_number):
            break
        print("Invalid card number. It should be 10 to 15 digits long.")
...
# Function to delete a membership plan (Admin)
def delete_plan():
    ...
    if plan:
        ...
        db.commit()
        print(f"Membership plan with ID {plan_id} has been deleted successfully.")
        return manage_membership_plans()
    else:
...
def borrow_books(user):
    ...
    while True:
        ...
        if not is_valid_account_number(account_number):
            print("Invalid card number. It must be 10-15 digits.")
        else:
            break
...
# Main Application
def main():
    print_header("Welcome to Endless Reading Vault")
